...
| 104 | Optical gate | Precision cut gap in a thin material. Gap shape not necessarily as shown in figure. |
...
| 403 | Shuttle start opening | Output signal of PTS (105) photoreceiver goes from level 406 to level 411 as shuttle opening starts to cross the PTS photo emitter (306) and photo receiver (305) light path. Output signal (400) of PRS (106) is at level 408.
| 404 | Shuttle end opening  | Output signal of PTS (105) goes from level 411 to level 406 as shuttle opening ends crossing of the PTS photo emitter (306) and photo receiver (305) light path. Output signal (400) of PRS (106) is at level 409.
...
Fit measured data to model (1) by solving a linear equation.

$$
X \cdot \mathbf{c} = \log(\mathbf{y}) \tag{2}
$$

In the case of (1), three measurement pairs $(x_0, y_0)$, $(x_1, y_1)$, $(x_2, y_2)$ enable solving for $c_0$, $c_1$, and $c_2$.
...
With these parameters, the calibration algorithm can solve for distance $x_{estimated}$ given a sensor output $y$:

$$x_{estimated} = \frac{-c_1 \pm \sqrt{c_1^2 - 4 c_2 (c_0 - \log(y))}}{2 c_2} \tag{4}$$

### Test model fit to sensor response

While the quadratic-exponential model (1) only needs three measurement pairs, test accuracy of the model by fitting (1) to a large number of measurement pairs. This involves solving with an optimization algorithm. The overdetermined system is:
...
Fit curve by minimizing the squared residual.
...
The horizontal axis is in units of seconds because the plot is response of sensor as the stepper motor moved the sensor toward a reflective surface (see the YouTube video link). The stepper motor moves the sensor at a fixed rate.
...
Assume measurement pairs $\{x, y\}$ at two points.
...
Given a sensor measured response $y$, the associated linearized value $x_{estimated}$ is:
...
When playing piano, the calibration algorithm computes $x_{estimated}$ from ADC output by application of (14).
...
8. Can get even better accuracy with quadratic exponential and with an additional precision cut gap for PTS optical gate (104) or (204) and value $x_2$ (for example test results show much better than the 2% error as described in the disadvantages section above). Or even as many curve-fitting sample points as desired.
